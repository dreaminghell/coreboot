...
/*
After write leveling for all ranks, check the PHY_CLK_WRDQS_SLAVE_DELAY result,
if the two ranks in one slice both met
"0x200-PHY_CLK_WRDQS_SLAVE_DELAY < 0x20 or 0x200-PHY_CLK_WRDQS_SLAVE > 0x1E0",
enable PHY_WRLVL_EARLY_FORCE_ZERO for this slice, and trigger write leveling again.
Else no additional write leveling is required
*/
static void check_write_leving_value(u32 channel,
				     const struct rk3399_sdram_params *sdram_params)
{
	struct rk3399_ddr_pi_regs *ddr_pi_regs = rk3399_ddr_pi[channel];
	struct rk3399_ddr_publ_regs *ddr_publ_regs = rk3399_ddr_publ[channel];
		...
		while (1) {
			...
			if ((((tmp >> 10) & 0x1) == 0x1) &&
				...
				break;
				 ...
				 (obs_err == 1))
				 printk(BIOS_DEBUG,
					"check_write_leving_value error!!!\n");
		}
		...
		for (i = 0; i < rank; i++) {
			...
			while (1) {
				...
				if ((((tmp >> 10) & 0x1) == 0x1) &&
				    ...
				    (obs_err == 0)) {
					if ((rank == 2) && (i == 1))
						check_write_leving_value(channel, sdram_params);
					break;
				} else if ((((tmp >> 4) & 0x1) == 0x1) ||
					 (obs_err == 1))
					return -1;
