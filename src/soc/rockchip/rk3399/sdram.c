...
#define DDR_STRIDE(n)			do { write32(&rk3399_pmusgrf->soc_con4,\
					(0x1F << (10 + 16)) | (n << 10));\
					} while (0)
...
#define INIT_PHY_TIMING_BEF_START	(1)
...
static void phy_pctrl_reset(struct rk3399_ddr_publ_regs *ddr_publ_regs,
			    u32 channel)
{
			 ...
			 u32 training_flag)
{
	...
	#if 0
	/*refer to simulation code,need confirm real need*/
	/*PI_TDFI_PHYMSTR_MAX_F0*/
	write32(&ddr_pi_regs->denali_pi[2], 0x000240b0);
	/*PI_TDFI_PHYUPD_RESP_F0[32,16] PI_TDFI_PHYMSTR_RESP_F2[15,0]*/
	write32(&ddr_pi_regs->denali_pi[7], 0xf0582058);
	/*why only channel 0 need set?*/
	if (channel == 0) {
		/*PI_TDFI_PHYUPD_TYPE0~3_F0*/
		write32(&ddr_pi_regs->denali_pi[8], 0x00000000);
		write32(&ddr_pi_regs->denali_pi[9], 0x00000000);
		write32(&ddr_pi_regs->denali_pi[10], 0x00000000);
		write32(&ddr_pi_regs->denali_pi[11], 0x00000000);
	}
	/*LPDDR4 init in F0*/
	if (sdram_params->dramtype == LPDDR4) {
		/*PI_TCAENT_F0*/
		clrsetbits_le32(&ddr_pi_regs->denali_pi[102],
				0x3fff << 16,
				0xfa << 16);
		/*PI_TVREF_LONG_F0*/
		clrsetbits_le32(&ddr_pi_regs->denali_pi[106],
				0x3ff,
				0xfa);
		/*PI_TCKEHDQS_F0*/
		clrsetbits_le32(&ddr_pi_regs->denali_pi[116],
				0x3f << 16,
				0xa << 16);
		/*PI_MR2_DATA_F0_0*/
		clrsetbits_le32(&ddr_pi_regs->denali_pi[126],
				0xffff,
				0x12);
		/*PI_MR2_DATA_F0_1*/
		clrsetbits_le32(&ddr_pi_regs->denali_pi[133],
				0xffff << 16,
				0x12 << 16);
	}
	#endif
	...
	if ((training_flag & PI_READ_GATE_TRAINING) == PI_READ_GATE_TRAINING) {
		for (i = 0; i < rank; i++) {
			...
			clrsetbits_le32(&ddr_pi_regs->denali_pi[80],
					0x3 << 24,
					0x2 << 24);
			/*PI_74 PI_RDLVL_GATE_REQ:WR:16:1,PI_RDLVL_CS:RW:24:2*/
			...
			while (1) {
				...
				tmp = read32(&ddr_pi_regs->denali_pi[174]) >> 8;
				/*check status obs*/
				/*PHY_43/171/299/427 PHY_GTLVL_STATUS_OBS_x:16:8*/
	...
	if ((training_flag & PI_READ_LEVELING) == PI_READ_LEVELING) {
		for (i = 0; i < rank; i++) {
			...
			while (1) {
				/*PI_174 PI_INT_STATUS:RD:8:18*/
				tmp = read32(&ddr_pi_regs->denali_pi[174]) >> 8;
				/*check status obs,status obs not report error bit*/
				/*PHY_46/174/302/430 phy_rdlvl_status_obs_X:16:8*/
				if ((((tmp >> 8) & 0x1) == 0x1) &&
				    (((tmp >> 13) & 0x1) == 0x1) &&
				    (((tmp >> 2) & 0x1) == 0x0))
					break;
	...
	if ((training_flag & PI_WDQ_LEVELING) == PI_WDQ_LEVELING) {
		for (i = 0; i < rank; i++) {
			/*need disable PI_WDQLVL_VREF_EN before wdq leveling?*/
