...
void rkclk_configure_cpu(enum apll_l_frequencies apll_l_freq)
{
	...
	assert((atclkm_div + 1) * ATCLKM_CORE_HZ ==
	       APLL_HZ && atclkm_div < 0x1f);

	pclk_dbg_div = APLL_HZ / PCLK_DBG_HZ - 1;
	assert((pclk_dbg_div + 1) * PCLK_DBG_HZ ==
	       APLL_HZ && pclk_dbg_div < 0x1f);
...
void rkclk_configure_spi(unsigned int bus, unsigned int hz)
{
	...
	if (bus == 3) {
		...
		assert((src_clk_div - 1 < 127) &&
		       (src_clk_div * hz == PPLL_HZ));
	} else { /* spi0,1,2,4,5 src clock from gpll*/
		src_clk_div = GPLL_HZ / hz;
		assert((src_clk_div - 1 < 127) &&
		       (src_clk_div * hz == GPLL_HZ));
	}
